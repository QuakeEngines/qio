...
void CSurfaceDlg::GetTexMods()
{
	...
	// a local copy of the texture matrix, given for a mtrAPI_i with width=2 height=2
	brushprimit_texdef_s	local_bp;
	brushprimit_texdef_s	*bpt;
	if (g_qeglobals.m_bBrushPrimitMode)
	{
		...
		else
    {
		...
		// the matrix returned must be understood as a mtrAPI_i with width=2 height=2
		FakeTexCoordsToTexMat( m_shift, m_rotate, m_scale, local_bp.coords );
		// copy the texture matrix in the global struct
		// fit the qtexture if we have a face selected, otherwise g_qeglobals.d_texturewin.brushprimit_texdef uses the basic mtrAPI_i with width=2 height=2
