...
BOOL CMapInfo::OnInitDialog() 
{
  ...
  int nValue = 0;
    ...
    m_nTotalEntities++;
    nValue = 0;
    mapEntity.Lookup(pEntity->getEntityClass()->getDeclName(), reinterpret_cast<void*&>(nValue));
    nValue++ ;
    mapEntity.SetAt(pEntity->getEntityClass()->getDeclName(), reinterpret_cast<void*>(nValue));
  }
